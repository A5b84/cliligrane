...
To compile on Windows, you can use the `GCC 12.4.0 (with POSIX threads) + MinGW-w64 12.0.0 (MSVCRT) - release 1` 64-bit version of [WinLibs](https://winlibs.com/).

 - Go inside data folder and clone the zxing-cpp repository :

```bash
  cd zxing-cpp/data/
  git clone https://github.com/zxing-cpp/zxing-cpp.git --depth 1 --recurse-submodules --shallow-submodules
```
